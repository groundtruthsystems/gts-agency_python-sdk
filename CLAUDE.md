...
- `annotations_client.py` / `annotations_dto.py` — publish a knowledge graph as annotator work (`/api/annotations` + `/api/annotation-specs` + `/api/annotation-workflows`, via `AgencyClient.annotations()`). **Publishing is three calls, not one**: `create_batch` (`POST /_command`, standard `{success,message,data:{id}}` envelope) leaves the batch in DRAFT with 0 jobs; `bind_workflow` attaches the workflow that governs its jobs — **without it the server refuses every job insert and the upload 500s** (the batch is created governed by nothing; issue #14); `upload_graph` (multipart `file`, 50 MiB cap) then materialises **one job per vertex matching `target_class`** (default `rule`) and flips the batch to ACTIVE — and its response body is `null`, so the job count comes from a `get_batch` read-back. `push_graph` chains resolve-workflow → create → bind → upload → read-back and is the agent-facing one-liner; the graph is the same `create.graph` payload sent to the ontology sandbox, accepted as a dict or a file path. Workflow ids are **resolved via `list_workflows`, never hardcoded** (system workflows are seeded per org); pass `workflow_id=` to override, and a 404 from that endpoint means a pre-workflow server, where the bind is skipped. Also `create_spec`/`get_spec`/`list_specs`, which seed the checklist a `job_type`'s jobs start from (**`get_spec`'s path segment is the `code`**, not the UUID). Batch job counters differ by server generation (`resolved`/`accepted`/`rejected_jobs` now, `completed_jobs` on older ones) so all four are optional. A failed upload leaves an empty DRAFT batch behind by design. See `docs/annotations.md`
