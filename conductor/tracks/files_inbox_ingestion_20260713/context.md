...
| ② | **this repo** | `feat/work-queue-delegate` | P1 done (`c5e8a89`); **P2+P2b DONE (`0318a6d`)**; rc12 bumped (`a982736`, not published); **P3A local e2e DONE — ALL STEPS PASSED vs `localhost:13001`, checkpoint `8502a49`** (both flat 409s evidenced live; `examples/quick_work_queue.py`). **2026-07-18: PR #10 MERGED to `main`** (merge `86cb337`; squashed 46→8 clean commits, Hermes review addressed in `7ed7625`). Both delegates now on `main`, `pyproject` there rc12. **NO publish** — merging doesn't publish (only a pushed `v*` tag does); last published = **rc11** (2026-07-07), no rc12 tag/run. **NEXT: P3B — publish still gated on ① merged + deployed to the SHARED platform** (re-e2e there → tag `v0.0.1rc12`, explicit go). **Guardrail overtaken by the merge:** `AgencySessionClient` (`fa542f3`) is now on `main` too, so an rc12 cut from `main` **bundles both** — accepted (③ adopted the session delegate + Gate-A-verified). `uv.lock` "blocker" was a misconception (gitignored, not in the wheel). **rc12 NOT published** (Gate B unmet) |
