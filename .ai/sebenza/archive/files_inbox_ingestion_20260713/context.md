...
| ② | **this repo** | `feat/work-queue-delegate` → **merged to `main`** | ✅ **COMPLETE (2026-07-18) — rc12 PUBLISHED.** P1/P2/P2b/P3A checkpointed; PR #10 merged (`86cb337`, Hermes review addressed `7ed7625`); tag `v0.0.1rc12` → CI "Build and Publish to PyPI" run 29650678800 SUCCESS → PyPI. Both flat 409s were evidenced live in 3A (`examples/quick_work_queue.py`) and under load inside ③'s worker. **rc12 bundles BOTH delegates** — `AgencySessionClient` (`fa542f3`) rode this branch and the merge co-located them on `main`, overtaking the "session own-rc" guardrail (accepted; ③ adopted it). Release flow: ② tag→publish (done) ∥ ① deploy → ③ pin-swap `==0.0.1rc12` + merge → ③ real-platform A2/B acceptance (downstream). `uv.lock` "blocker" was a misconception (gitignored) |
